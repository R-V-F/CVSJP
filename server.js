...
async function main() {
    ...
    let lives_list = await getLives(channels_list, check_lives_driver);
    let lives_driver_list = await loadStreams(lives_list);

    await addRow(lives_driver_list, con);
    ...
    app.listen(port, async () => {
        ...
        setInterval(async () => {
            let new_lives_list = await getLives(channels_list, check_lives_driver);
            if(!equalsCheck(lives_list, new_lives_list)) {
                let temp_list_of_drivers = await deleteInvalidDrivers(lives_list,new_lives_list,lives_driver_list)
                await addNewDrivers(lives_list,new_lives_list,temp_list_of_drivers);
                lives_driver_list.length = 0;
                for(driver of temp_list_of_drivers) lives_driver_list.push(driver);
                lives_list.length = 0;
                for(live of new_lives_list) lives_list.push(live)
            }
            await addRow(lives_driver_list, con);
        }
        ,240000);
